    ...
    ui(new Ui::MainWindow)
{
   ...
   // Прикрепляем виджеты к краям экрана
    ...
    ui->pushButton_19->setIcon(QIcon("pic/play.png"));
    ui->pushButton_19->setIconSize(QSize(35,35));
    ...
    LoadTemplate("source/Template/define_var2.txt");
...
void MainWindow::on_pushButton_9_clicked()
{
    if ((ui->lineEdit_3->text()!="")&&(ui->lineEdit_4->text()!=""))
    {
        ...
        if (ui->comboBox->itemText(ComboSelItem)=="=")
        {
            S+="==";
        }
        else
...
void MainWindow::on_pushButton_12_clicked()
{
    if ((ui->lineEdit_5->text()!="")&&(ui->lineEdit_6->text()!=""))
    {
        ...
        if (ui->comboBox_2->itemText(ComboSelItem)=="=")
        {
            S += "==";
        }
        else
      ...
      //Записываем в файл данные, если X=
        if (item->child(i)->text(0).count(":=")>0)
        {
            ...
            if (S.length()>3)
            {
                ...
                parametr2=S;
                index=S.indexOf("=");
                parametr1.remove(index-2,S.length());
      ...
      //Записываем в файл данные, если if
        if (item->child(i)->text(0).count("if")>0)
        {
            ...
            if (S.length()>2)
            {
                ...
                parametr1.remove(index,S.length());
                parametr3.remove(0,index+parametr2.length());
                parametr3.remove(parametr3.length()-1,1);
                Out[CountOut].operator +=(QString::number(parametr1.length()));
                Out[CountOut].operator +=(" ");
                Out[CountOut].operator +=(parametr1);
...
void MainWindow::on_pushButton_16_clicked()
{
    ...
    else
   {
    ...
    char* str = (char*)path.toStdString().c_str();
    SaveCode(c, str);
...
void MainWindow::on_pushButton_19_clicked()
{
    QString s = path;
    s+= ".exe";
    char* str = (char*)s.toStdString().c_str();
    EasyRunExe(str,"");
